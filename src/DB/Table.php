...
class Table {
     ...
     *
     * @param string $pkVal The value of the PK of the record to get.
     * @return Record|false The record object, or false if it wasn't found.
     */
    public function get_record($pkVal) {
        ...
        $sql = "SELECT " . $this->columns_sql_select() . " "
                . "FROM `" . $this->get_name() . "` "
                . "WHERE `" . $pk_column->get_name() . "` = :id "
                . "LIMIT 1";
        $row = $this->database->query($sql, ['id'=>$pkVal])->fetch();
    ...
    public function get_title() {
        return \App\App::titlecase($this->get_name());
    }
    ...
    public function count_records() {
        if (!$this->record_count) {
            ...
            $params = $this->apply_filters($sql);
            $count = $this->database->query($sql, $params)->fetchColumn();
