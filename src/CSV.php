...
class CSV {
    ...
    private function save_file($uploaded) {
        ...
        $this->hash = md5(time());
        rename($uploaded['file'], sys_get_temp_dir() . '/' . $this->hash);
    }
     ...
     * @param \App\DB\Database $db The database instance.
     * @param \App\DB\Table $table The table into which to import data.
     * @param string[] $column_map array of DB names to import names.
     * @return integer The number of rows imported.
     */
    public function import_data(DB\Database $db, $table, $column_map) {
        $change_tracker = new DB\ChangeTracker($db);
