...
class Table
{
    ...
    /** @var \SWFW\DB\Database The database to which this table belongs. */
    protected $database;
...
//                }
    ...
    public function getRows($with_pagination = true, $save_sql = false)
    {
        ...
        if ($with_pagination) {
            $rowsPerPage = \SWFW\Settings::get('rows_per_page', 30);
        ...
        $rows = $this->database->query($sql, $params);
    ...
    public function getRecord($id)
    {
        ...
        $sql = "SELECT * FROM `" . $this->getName() . "` "
                ...
                . "LIMIT 1";
        $this->database->setFetchMode(\PDO::FETCH_ASSOC);
        $row = $this->database->query($sql, array($pk_name => $id));
        $this->database->setFetchMode(\PDO::FETCH_OBJ);
        return $row->fetch();
    }
...
//        }
        ...
        foreach ($data as $field => $value) {
            ...
            /*
             * Numbers
             */ elseif (!is_numeric($value) && (substr($column->get_type(), 0, 3) == 'int' || substr($column->get_type(), 0, 7) == 'decimal' || substr($column->get_type(), 0, 5) == 'float')
            ) {
            ...
            /*
             * Dates & times
             */ elseif (($column->get_type() == 'date' || $column->getType() == 'datetime' || $column->getType() == 'time') && $value == '') {
                $data[$field] = null;
            }
        }
